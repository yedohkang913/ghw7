...
double r, int step ) {
	...
	for ( lat = latStart; lat < latStop; lat++ ) {
		for ( longt = longStart; longt <= longStop; longt++ ) {
			...
			y0 = points->m[1][index];
			z0 = points->m[2][index];
			x1 = points->m[0][(index + 1) % points->lastcol];
			y1 = points->m[1][(index + 1) % points->lastcol];
			z1 = points->m[2][(index + 1) % points->lastcol];
			x2 = points->m[0][(index + step) % points->lastcol];
			y2 = points->m[1][(index + step) % points->lastcol];
			z2 = points->m[2][(index + step) % points->lastcol];
			x3 = points->m[0][(index) % points->lastcol];
			y3 = points->m[1][(index) % points->lastcol];
			z3 = points->m[2][(index) % points->lastcol];
			x4 = points->m[0][(index + step - 1) % points->lastcol];
			y4 = points->m[1][(index + step - 1) % points->lastcol];
			z4 = points->m[2][(index + step - 1) % points->lastcol];
			
			add_polygon(edges, x0, y0, z0, x1, y1, z1, x2, y2, z2);
			add_polygon(edges, x3, y3, z3, x2, y2, z2, x4, y4, z4);
		}
	}
	free_matrix(points);
}
